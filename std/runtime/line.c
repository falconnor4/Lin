/* ============================================================================
 * Lin `.line` binary container runtime (std — not part of the src/ core).
 *
 * #included into src/compile.c (unity build): serializes/deserializes a
 * reduced Net to a self-running .line executable (shebang re-invokes the exact
 * engine that produced it, instead of a PATH-ambiguous `#!/usr/bin/env lin`).
 * Shared statics/structs (Net, Port, Scope, net_init, net_free) come from the
