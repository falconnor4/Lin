...
static Port ct_splice(Def *d, Scope sc) {
  ...
  for (int i = 0; i < n; i++) {
    map[i] = net_alloc(N, s->tag[i], scope_rebase(N, s, lvl, s->scope[i])).node;
  }
...
static Port ct_in(Term *t, Scope sc) {
  ...
  case TLAM: {
    /* The binder's SOURCE name is not stamped anywhere: it is not part of what the net means
       (α-equivalent nets are one net), so the printer generates a name at render time and every
       reader states the encoding it expects instead of reading a label. */
    Port self = net_alloc(N, LAM, fan_lvl());
...
int net_load_line(Net *n, const char *path) {
  ...
  for (uint32_t k = 0; k < nnamed; k++) {
    ...
    char skip[256];
    if (fread(skip, 1, len, f) != len) { fclose(f); net_free(n); return 0; }
  }
  if (nlv > 0) {
  ...
  if (ver == 4) {
    ...
    if (nf > 0) {
      ...
      if (!lin_driver_carry_legacy(n, DT_FLOAT, d, (size_t)nf * sizeof(double)))
        fprintf(stderr, "lin: this artifact carries a float table but no driver provides the float "
                        "domain -- load std/drivers/values.lin\n");
      free(d);
    }
  } else if (ver >= 5) {
    uint32_t cnt = 0;
    if (fread(&cnt, sizeof cnt, 1, f) != 1) { fclose(f); net_free(n); return 0; }
    lin_driver_carry_read(n, f, cnt);
  }
  fclose(f);
